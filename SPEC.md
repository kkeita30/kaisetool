...
`index.html` は、動画に合成する「解説ボード」画像（1920×1080px、背景透過PNG）をチャプターごとに書き出すためのスタンドアロンWebツールです。ビルドステップなし。ブラウザで開くだけで動作します。
...
1. 起動時に同フォルダの `boards.json` を自動読み込み → チャプター一覧がドロップダウンに展開
2. 別ファイルを読む場合はファイル選択UIから読み込み可能
3. チャプターを選択 → タイトル・テキスト行がフォームに読み込まれる
4. フォームで内容を編集（行タイプの変更、テキスト編集、行の追加・削除・並び替え）
5. チャプターの追加・削除も可能（「＋ チャプターを追加」「このチャプターを削除」ボタン）
6. プレビューで確認（右側に1920×1080を0.5倍縮小表示）
7. 「PNG書き出し」→ 現在のチャプターを1枚ダウンロード
8. 「全チャプター一括書き出し」→ 全チャプターを連番PNGで順番にダウンロード
9. 「JSONを保存」→ 全チャプターの編集内容を `boards.json` としてダウンロード
...
| 形式 | PNG |
| 背景 | 透過（アルファチャンネルあり） |
| ファイル名 | 単体: `board_{タイトル}.png` / 一括: `board_{連番}_{タイトル}.png` |
...
- `index.html` はReact + JSX をBabelでトランスパイルしています（`<script type="text/babel">`）。ビルド不要ですが、`file://` プロトコルでは `boards.json` の自動読み込みが動作しないため、**VS Code の Live Server 拡張**で起動すること。
- boards.json の内部マーカー（`▼`/`⇒`/`　`）は固定。`midashiPrefix`/`pointPrefix` は表示専用で、変更してもデータ形式には影響しない。
